...
def calculate_efficiency(classification_result, matrices_all, training_set):
    ...
    for i in range(0, len(training_set_a_t)):
        ...
        elif classification_result[i] == 1:
            if training_set_a_t[i] in matrices_all[1]:
                print("Weszlo2")
                #efficiency = efficiency + 1
        else:
            efficiency +=1
    ...
    for i in range(0, len(training_set_b_t)):
        if classification_result[i + len(training_set_a_t)] == 0:
            if training_set_b_t[i] in matrices_all[0]:
                print("Weszlo3")
        elif classification_result[i + len(training_set_a_t)] == 1:
